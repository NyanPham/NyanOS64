...
__attribute((aligned(0x10))) static idt_entry_t idt[IDT_MAX_DESCRIPTORS];
...
void exception_handler(registers_t *regs)
{
    if (regs->int_no == 14) // page fault
    {
        ...
        uint8_t is_user = (regs->cs & 0x3) == 3 ? 1 : 0;
        uint8_t fail_to_cow = 0;

        uint64_t err_code = (regs->err_code & 0x7);

        if (err_code == 0x7 || err_code == 0x03)
        {
            if (vmm_handle_cow(fault_addr) == 0)
            {
                return;
            }

            kprint("[SEGFAULT] CoW FAILED (Return != 0)!\n");
            fail_to_cow = 1;
        }

        if (is_user || fail_to_cow)
        {
            kprint("\n[SEGFAULT] User Process tried to access invalid memory: ");
            kprint_hex_64(fault_addr);
            kprint(" | Err_code: ");
            kprint_hex_64(regs->err_code);
            kprint("\nIs user: ");
            kprint_hex_64(is_user);
            kprint("\nFail to cow: ");
            kprint_hex_64(fail_to_cow);
            kprint("\nKilling process...\n");
